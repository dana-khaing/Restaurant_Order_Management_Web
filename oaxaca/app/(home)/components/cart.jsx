          ...
          <div className="flex flex-col gap-4 mt-2">
            {cartItems?.map((item) => (
              <div key={item.id} className="flex ">
                <img className="w-16 h-16 rounded-lg" src="/images/burrito.jpeg" alt="Item" />
                <div className="flex justify-between w-full pl-2">
                  <div className="flex flex-col gap-1">
                    <span className="font-semibold">{item.name}</span>
                    <div className="flex gap-2 items-center">
                      <span onClick={() => decrementQuantity(item.productId)} className="bg-gray-500 rounded-lg px-2 py-0.5 text-white cursor-pointer">
                        -
                      </span>
                      <span>{item.quantity}</span>
                      <span onClick={() => incrementQuantity(item.productId)} className="bg-gray-500 rounded-lg px-2 py-0.5 text-white cursor-pointer">
                        +
                      </span>
                    </div>
                  </div>
            ...
            <span className="text-right text-lg font-semibold">
              Total: £{cartItems?.reduce((prev, item) => prev + item.quantity * item.price, 0).toFixed(2) || 0}
            </span>
          </div>
